  ...
  <div class="mod-config">
    ...
    <el-pagination
      ...
      layout="total, sizes, prev, pager, next, jumper">
		...
		<!-- 弹窗 - 信息项 -->
    <item-dialog v-if="innerVisible" ref="itemDialog" @refreshDataList="getDataList"></item-dialog>
  </div>
</template>
	...
	methods: {
		...
		getDataList() {
			...
			if (this.dataForm.resultId) {
				...
				API.information.itemList(params).then(({ data }) => {
		...
		addOrUpdateHandle(id, isEdit) {
			...
			this.$nextTick(() => {
				this.$refs.itemDialog.init(id, isEdit);
			});
		},
		// 删除
		deleteHandle(id) {
			...
			this.$confirm(
				...
				API.information.itemDelete(ids).then(({ data }) => {
					if (data && data.code === 0) {
						this.$message({
							message: '操作成功',
							type: 'success',
